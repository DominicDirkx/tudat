...
BOOST_AUTO_TEST_SUITE( test_n_way_differenced_range_model )
...
BOOST_AUTO_TEST_CASE( testNWayDifferencedRangeModel )
{
    // Load Spice kernels
    ...
    linkEnds[ reflector ] = std::make_pair( "Mars" , ""  );
    ...
    boost::shared_ptr< ObservationSettings > rangeRateObservableSettings = boost::make_shared<
            NWayDifferencedRangeRateObservationSettings >(
                boost::shared_ptr< LightTimeCorrectionSettings >( ), 3, boost::bind( &integrationTimeFunction, _1 ) );
    ...
    // Create range rate observation settings and model
    boost::shared_ptr< ObservationSettings > rangeObservableSettings = boost::make_shared< NWayRangeObservationSettings >(
                boost::shared_ptr< LightTimeCorrectionSettings >( ), 3 );
    ...
    for( unsigned testCase = 0; testCase < 3; testCase++ )
    {
        for( double observationTime = 86400.0; observationTime <= 86400.0; observationTime += observationTime )
        {
            ...
            for( unsigned linkCompare = 0; linkCompare < 2; linkCompare++ )
            {
                ...
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( rangeEndLinkEndStates.at( linkCompare ), rangeRateLinkEndStates.at( linkCompare + 4 ), 1.0E-15 );
                BOOST_CHECK_CLOSE_FRACTION( rangeEndLinkEndTimes.at( linkCompare ), rangeRateLinkEndTimes.at( linkCompare + 4 ), 1.0E-15 );
           }
