...
BOOST_AUTO_TEST_CASE( testScaledStatePropagation )
{
    ...
    double simulationStartEpoch = 1.0 * 86400.0;
    double simulationEndEpoch = 6.0 * 86400.0;
    ...
    {
        bodySettings[ bodyNames.at( i ) ]->rotationModelSettings = std::make_shared< SimpleRotationModelSettings >(
                    "ECLIPJ2000", "IAU_" + bodyNames.at( i ),
                    spice_interface::computeRotationQuaternionBetweenFrames(
                        "ECLIPJ2000", "IAU_" + bodyNames.at( i ), 0.0 ),
    ...
    for( int useSphericalHarmonicGravity = 0; useSphericalHarmonicGravity < 2; useSphericalHarmonicGravity++ )
    {
        ...
        for( int i = 0; i < numberOfNumericalBodies; i++ )
        {
            ...
            for( int j = 0; j < numberOfBodies; j++ )
            {
                if( i != j )
                {
                    if( bodyNames.at( j ) == "Jupiter" )
                    {
                        if( useSphericalHarmonicGravity == 1 )
                        {
        ...
        for( int useScaledAccelerations = 0; useScaledAccelerations < 2; useScaledAccelerations++ )
        {
            ...
            if( useSphericalHarmonicGravity )
            {

                parameterNames.push_back( std::make_shared< EstimatableParameterSettings >( "Europa", constant_rotation_rate ) );

                parameterNames.push_back( std::make_shared< EstimatableParameterSettings >( "Europa", rotation_pole_position ) );
                parameterNames.push_back( std::make_shared< EstimatableParameterSettings >( "Jupiter", rotation_pole_position ) );
            ...
            tudat::estimatable_parameters::printEstimatableParameterEntries(
            ...
            for( int i = 0; i < numberOfNumericalBodies; i++ )
            {
                ...
                for( int j = 0; j < numberOfNumericalBodies; j++ )
                {
                    ...
                    for( int k = 0; k < 3; k++ )
                    {
                        for( int l = 0; l < 3; l++ )
                        {
                            BOOST_CHECK_SMALL( std::fabs( blockDifference( k, l ) ), 1.0E-10 * positionPositionNorm );
                            BOOST_CHECK_SMALL( std::fabs( blockDifference( k, l + 3 ) ), 1.0E-10 * positionVelocityNorm );

                            BOOST_CHECK_SMALL( std::fabs( blockDifference( k + 3, l ) ), 1.0E-10 * velocityPositionNorm );
                            BOOST_CHECK_SMALL( std::fabs( blockDifference( k + 3, l + 3 ) ), 1.0E-10 * velocityVelocityNorm );
                ...
                for( int j = 0; j < numberOfParameters - 6 * numberOfNumericalBodies; j++ )
                {
                    ...
                    Eigen::Vector6d secondBlock =
                            sensitvityTransitionHistories.at( 1 )[ stateIterator.first ].block( i * 6, j, 6, 1 );
                    ...
                    double toleranceMutliplier = ( j > 1 && j < 7 ) ? 2.0E5 : 1.0;

                    for( int k = 0; k < 3; k++ )
                    {
                        BOOST_CHECK_SMALL( std::fabs( blockDifference( k ) ), std::max( 1.0E-10 * toleranceMutliplier * positionNorm, 1.0E-300 ) );
                        BOOST_CHECK_SMALL( std::fabs( blockDifference( k + 3 ) ), std::max( 1.0E-10 * toleranceMutliplier * velocityNorm, 1.0E-300 ) );
